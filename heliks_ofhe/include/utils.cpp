...
//Samples (dim0, dim1) 2D vector from a Uniform Integer Distribution between (min, max).
vector<vector<int64_t>> gen2D_UID_int64(int dim0, int dim1, int min, int max){
    random_device rnd_device;
    mt19937 mersenne_engine {rnd_device()};
    uniform_int_distribution<int> dist {min, max};
...
//Samples (dim0, dim1) 2D vector with elements in row-major ascending order.
vector<vector<int64_t>> gen2DinRange_int64(int dim0, int dim1){
    vector<vector<int64_t>> out;
    for(int i = 0; i < dim0; i++){ 
        vector<int64_t> row;
        for(int j = 0; j<dim1; j++) row.push_back(i*dim1+j+1);
...
//Samples (dim0, dim1) 2D vector with elements in row-major ascending order.
vector<vector<int32_t>> gen2DinRange_int32(int dim0, int dim1){
...
//Prints 1D vector.
void print1D(vector<int64_t> inp1D, int num_digits, int max_row){
    if (max_row <= 0) max_row += inp1D.size();
    auto print_row = max_row > inp1D.size() ? inp1D.size() : max_row;

    for(int i{}; i < print_row; i++)
        printf("%+ld ", inp1D.at(i));
...
//Prints 2D vector.
void print2D(vector<vector<int64_t>> inp2D, int num_digits, int max_row, int max_col){
    auto print_col = max_col > inp2D.size() ? inp2D.size() : max_col;
    for(int i = 0; i < print_col; i++){
        print1D(inp2D.at(i), num_digits, max_row);
    }
}
...
//Prints 3D vector.
void print3D(vector<vector<vector<int64_t>>> inp3D, int num_digits, int max_row){
    for(auto mat: inp3D){
        print2D(mat, num_digits, max_row);
        cout << endl;
    }
...
//Sums all elements of a 2D vector.
int64_t sum2D(vector<vector<int64_t>> inp2D){
    double out{};
    for(auto row: inp2D)
    for(auto val: row)
    out = out + val;
...
//Pads 2D vector to [(padT + dim0 + padB), (padL + dim1 + padR)], i.e adds padT rows on top, padB rows
//in bottom, padL columns on left and padR columns on right.
vector<vector<int64_t>> pad2D(vector<vector<int64_t>> inp2D, int padT, int padB, int padL, int padR){
    vector<vector<int64_t>> out (padT+padB, vector<int64_t>(inp2D.at(0).size(), 0));
    out.insert(out.begin()+padT, inp2D.begin(), inp2D.end());
    for(int i = 0; i < out.size(); i++){
        out.at(i).insert(out.at(i).begin(), padL, 0);
        out.at(i).insert(out.at(i).end(), padR, 0);
...
vector<vector<int64_t>> rotate2D(vector<vector<int64_t>> inp2D, int nRotRows, int nRotCols){
...
// from range X to Y
// source: https://www.geeksforgeeks.org/slicing-a-vector-in-c/
vector<int64_t> slice1D(vector<int64_t>& arr,
                    int X, int Y)
{
    if(Y<=0) Y = arr.size() + Y - 1;
...
vector<int64_t> rowEncode2D(vector<vector<int64_t>> inp2D){
    vector<int64_t> data;
    for(auto row: inp2D){
        data.insert(data.end(), row.begin(), row.end());
    }
...
int64_t float_to_fixed(double flt_val, double scale){
    return (int64_t) round(flt_val*scale);
}

double fixed_to_float(int64_t fxd_val, double scale){
...
// Taken from SEAL/native/src/seal/util/numth.h
